...
\begin{figure}[H]
    ...
    \label{fig:1}
\end{figure}
	% \item Sample $p^{(0)}$ from a Dirichlet with parameters $\gamma$;
	% \item Sample each $\mu_k^{(0)} \sim N(m, \tau)$ and each
	% $(\sigma_k^2)^{(0)} \sim IG(\alpha, \beta)$;
	% \item Sample each $Z_i^{(0)} \mid p^{(0)}$, which is
	% $P(Z_i^{(0)} = k \mid p^{(0)}) = p_k^{(0)}$,
	% for values in $\{1,...,K\}$;
	% \item Sample each $X_i^{(0)} \mid Z_i^{(0)} = z_i,
	% p^{(0)},\mu^{(0)}, (\sigma^2)^{(0)} \sim
	% N (\mu_{z_i}^{(0)}, (\sigma_{z_i}^2)^{(0)})$
