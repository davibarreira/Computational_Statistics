...
\begin{enumerate}[leftmargin=!,labelindent=5pt]
	...
	$$\mathbb{P}(\text{X is accepted in step(b)}) =
	 ...
	 % \begin{itemize}
	 % 	\item Generate $u \sim Unif[0,1], x \sim q$ 
	 % 	\item If $u \leq \frac{h(x)}{M\tilde{q}(x)}$, accept $x$;
	 % 	\item Else, generate $v \sim Unif[0,1]$;
	 % 	\item Check if
	 % 	$v \leq \frac{\tilde{\pi}(x)-h(x)}{M\tilde{q}(x) - h(x)}$;
	 % 	\item I
